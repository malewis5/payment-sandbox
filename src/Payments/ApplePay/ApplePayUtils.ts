...
export const createLineItems = (
  ...
  if (shipping) {
    ...
    lineItems.push({ label: 'Shipping', amount: shippingMethods[0].amount, type: 'final' })
  }
  if (tax) {
...
export const createPaymentRequest = (
  ...
  if (applePayInstance) {
    ...
    //@ts-ignore
    const paymentRequest: ApplePayJS.ApplePayPayment = applePayInstance.createPaymentRequest({
      total: {
        label: storeName,
...
export const calculateApplePayTotal = (subtotal: string, shipping?: string, tax?: string): string => {
...
export const createApplePaySession = (
  ...
  const session: ApplePaySession = new ((window as unknown) as ApplePayJS.ApplePayWindow).ApplePaySession(
    3,
    paymentRequest,
  )
  ...
  session.onpaymentauthorized = function(event: ApplePayJS.ApplePayPaymentAuthorizedEvent) {
    if (applePayInstance) {
      applePayInstance.tokenize(
        ...
        function(tokenizeErr, payload) {
          if (tokenizeErr) {
            session.completePayment(((window as unknown) as ApplePayJS.ApplePayWindow).ApplePaySession.STATUS_FAILURE)
            throw Error(tokenizeErr.message)
          }
          // Send payment nonce to micro-service
          const postNonce = async (nonce: string, applePayPayment: ApplePayJS.ApplePayPaymentRequest) => {
            ...
            fetch('https://payment-microservice.ngrok.io/payment-nonce', requestOptions).then(response => {
              if (response.status === 200) {
                session.completePayment(
                  ((window as unknown) as ApplePayJS.ApplePayWindow).ApplePaySession.STATUS_SUCCESS,
                )
                onPaymentSuccess(response)
              } else {
                session.completePayment(
                  ((window as unknown) as ApplePayJS.ApplePayWindow).ApplePaySession.STATUS_FAILURE,
                )
  ...
  if (shippingHandler) {
    session.onshippingmethodselected = function(event: ApplePayJS.ApplePayShippingMethodSelectedEvent) {
      ...
      const newTotal = calculateApplePayTotal(prevSubtotal, newShipping, prevTax)

      const update: ApplePayJS.ApplePayShippingMethodUpdate = {
        newTotal: { amount: newTotal, label: paymentRequest.total.label },
        newLineItems: createLineItems(prevSubtotal, newShipping, paymentRequest.shippingMethods, prevTax),
      }
  ...
  if (shippingHandler || taxHandler) {
    session.onshippingcontactselected = function(event: ApplePayJS.ApplePayShippingContactSelectedEvent) {
      const shippingMethods = shippingHandler ? shippingHandler(event) : undefined
      ...
      const update: ApplePayJS.ApplePayShippingContactUpdate = {
        ...
        newLineItems: createLineItems(prevSubtotal, shipping, shippingMethods, tax),
      }
      // Update Payment Request with new items.
      paymentRequest.total = update.newTotal
      paymentRequest.lineItems = update.newLineItems
